...
public class VirtualScapeMapReader {
	...
	private Map toMap(VirtualScapeMap virtualScapeMap) {
		Map map = new Map();
	...
	private void addDecorToMap(VirtualScapeTile decor, Map map) {
		...
		if (type != null) {
			...
			if (type == VirtualScapeDecorType.BRUSH_1 || type == VirtualScapeDecorType.PALM_1) {
				map.addTile(TileType.SWAMP, x + tileAttachementOffset.x, y + tileAttachementOffset.y, decor.getPosZ() + tileAttachementOffset.z);
			}
		}
	}
