...
import fr.lyrgard.hexScape.model.TileType;
...
//	static final float top = HEX_SIZE_Y;
	...
	public static List<Vector2f> getTexCoord(Direction direction, TileType type) {
		...
		switch (direction) {
		case TOP:			
			texCoord.add(type.getTopTexture().getCoord(0.5f,0));
			texCoord.add(type.getTopTexture().getCoord(1,0.25f));
			texCoord.add(type.getTopTexture().getCoord(1,0.75f));
			texCoord.add(type.getTopTexture().getCoord(0.5f,1));
			texCoord.add(type.getTopTexture().getCoord(0,0.75f));
			texCoord.add(type.getTopTexture().getCoord(0,0.25f));
			break;
		case BOTTOM:
			// nothing to do here
			break;
		...
		case WEST:
		case NORTH_WEST:
			texCoord.add(type.getSideTexture().getCoord(1,0));
			texCoord.add(type.getSideTexture().getCoord(0,0));
			texCoord.add(type.getSideTexture().getCoord(1,1));
			texCoord.add(type.getSideTexture().getCoord(0,1));
			break;
		}
		return texCoord;
	}
