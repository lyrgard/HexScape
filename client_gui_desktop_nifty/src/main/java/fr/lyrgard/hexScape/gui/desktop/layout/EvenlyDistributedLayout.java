 ...
 * @author void
 */
public class EvenlyDistributedLayout implements LayoutManager {
	...
	public final void layoutElements(final LayoutPart root, final List < LayoutPart > children) {
		...
		int numberInRow = (int)Math.ceil(Math.sqrt(children.size()));
		int numberOfRow = (int)Math.ceil(children.size()/(double)numberInRow);
		
		int rowHeigth = rootBoxHeight/(numberOfRow);
		...
		for (int i = 0; i < children.size(); i++) {
			...
			int elementWidth = calcElementWidth(children, rootBoxWidth, boxConstraints, 0);
			...
			int y = rootBoxY + (int)((currentRowNumber - 0.5f) * rowHeigth - (box.getHeight()/2));
			box.setY(y);

			int columnWidth;
			int numberOfItemLeftFromStartOfRow = children.size() - ((currentRowNumber - 1) * numberInRow);
			if (numberOfItemLeftFromStartOfRow >= numberInRow) {
				columnWidth = rootBoxWidth/(numberInRow);
			} else {
				columnWidth = rootBoxWidth/(numberOfItemLeftFromStartOfRow);
			}
			int x = rootBoxX + (int)((placeInRow - 0.5f) * columnWidth - (box.getWidth()/2));
			box.setX(x);
			
			placeInRow++;
			if (placeInRow > numberInRow) {
				placeInRow = 1;
				currentRowNumber++;
			}
		}
